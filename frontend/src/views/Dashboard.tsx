...
import { BsFileEarmarkText, BsFileEarmarkImage, BsFileEarmarkPdf, BsFileEarmarkWord, BsFileEarmarkExcel, BsFileEarmarkZip } from "react-icons/bs";
...
// Small wrapper so each stat can call useCountUp independently
...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  const recentActivity = [...activityData]
    ...
    .slice(0, 5) // cap recent activity to max 5
    .map((activity) => {
  ...
  const getFileTypeIcon = (type: string): any => {
    ...
    if (lowerType.includes('image') || lowerType.includes('png') || lowerType.includes('jpg') || lowerType.includes('jpeg') || lowerType.includes('gif')) return BsFileEarmarkImage;
    if (lowerType.includes('doc') || lowerType.includes('txt')) return BsFileEarmarkWord;
    if (lowerType.includes('xls') || lowerType.includes('csv') || lowerType.includes('spreadsheet')) return BsFileEarmarkExcel;
    if (lowerType.includes('zip') || lowerType.includes('rar') || lowerType.includes('archive')) return BsFileEarmarkZip;
  ...
  // Always render both AES variants so a 100/0 split still shows both charts.
  const aesBase = [
    { type: 'AES-128', color: 'from-[#b2f7ef] to-[#97eeff]' },
    { type: 'AES-256', color: 'from-[#f7d6e0] to-[#f2b5d4]' },
  ]
  const aesUsageData = aesBase.map((base) => {
    const found = encryptionBreakdown.find((item) => item.type === base.type)
    return {
      type: base.type,
      count: found?.count ?? 0,
  ...
  const expiringSoonCount = (dashboardStats as { expiring_soon_files?: number } | null)?.expiring_soon_files ?? 0;
  const storageUsedMb = dashboardStats?.storage_used_mb ?? 0;
  const storageUsagePct = storageUsedMb > 0
    ? Math.min(100, (storageUsedMb / (storageUsedMb + 1024)) * 100)
    : 0; // soft scale with no fixed storage limit

  // Scroll-triggered reveal refs for each dashboard section
  const [quickStatsRef, quickStatsVisible] = useRevealOnScroll<HTMLDivElement>();
  const [securityRef, securityVisible] = useRevealOnScroll<HTMLDivElement>();
  const [aesRef, aesVisible] = useRevealOnScroll<HTMLDivElement>();
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-6">
      ...
      {/* Body-only loader (keeps header visible) */}
      {loading ? (
        <div className="flex flex-1 items-center justify-center rounded-2xl border-2 border-gray-200/80 bg-white/70 min-h-[360px]">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold bg-gradient-to-r from-[#084d45] via-[#0a6d5f] to-[#084d45] bg-clip-text text-transparent mb-2">
              Loading Dashboard
            </h2>
            <p className="text-gray-500 text-sm">Fetching your encrypted files data...</p>
          ...
          {stats.map((stat, index) => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900"><CountUpValue value={stat.value} /></p>
                {stat.subtitle && (
      ...
      {!loading && (
      <div ref={quickStatsRef} className={`grid grid-cols-1 lg:grid-cols-5 gap-6 ${quickStatsVisible ? 'reveal-visible' : 'reveal-hidden'}`}>
            ...
            {quickStats.map((stat, index) => (
              <div key={index} className="p-4 rounded-xl bg-gradient-to-br from-gray-50 to-gray-100/50 border-2 border-gray-200/50 hover:shadow-md transition-all duration-300 hover:-translate-y-1">
                <div className="flex flex-col items-center text-center">
                  ...
                  <CountUpValue value={stat.value} className="text-3xl font-bold text-gray-900 mb-1" />
          ...
          <div className="p-5 rounded-xl bg-gradient-to-br from-[#b2f7ef]/10 to-[#97eeff]/10 border-2 border-[#b2f7ef]/30 mb-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-sm font-bold text-[#084d45]">
                {dashboardStats ? formatStorage(dashboardStats.storage_used_mb) : '0 MB'}
              </span>
            </div>
      ...
      {!loading && (
      <div ref={securityRef} className={`bg-white rounded-2xl p-6 border-3 border-gray-200/80 shadow-sm ${securityVisible ? 'reveal-visible' : 'reveal-hidden'} ${!hasAnimated ? 'animate-in fade-in duration-700' : ''}`} style={!hasAnimated ? { animationDelay: '800ms', animationFillMode: 'backwards' } : {}}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {expiringSoonCount === 0 ? 'No expiring files' : `${expiringSoonCount} expiring soon`}
                </p>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between p-4 rounded-xl bg-orange-50 border-2 border-orange-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-900">Expiring Soon</p>
                <p className="text-sm text-gray-600">Upcoming expirations</p>
              </div>
            </div>
            {/* separate expired vs expiring logic */}
            <span className="text-2xl font-bold text-orange-700">{expiringSoonCount}</span>
          </div>
        </div>
      </div>
      )}
      ...
      {!loading && (
      <div ref={aesRef} className={`grid grid-cols-1 lg:grid-cols-2 gap-6 ${aesVisible ? 'reveal-visible' : 'reveal-hidden'}`}>
                  ...
                  <div className="relative w-36 h-36">
                    ...
                    <svg className="w-36 h-36 transform -rotate-90" viewBox="0 0 100 100">
                      ...
                      <circle
                        ...
                        strokeWidth="8"
                        strokeDasharray={`${data.percentage > 0 ? data.percentage * 2.64 : 2} ${data.percentage > 0 ? 264 - data.percentage * 2.64 : 262}`}
                        strokeLinecap="round"
                        className="transition-all duration-1500 ease-out"
                        style={{ 
                          animationDelay: `${index * 0.3}s`,
                          filter: 'drop-shadow(0 0 8px rgba(178, 247, 239, 0.4))',
                          opacity: data.percentage > 0 ? 1 : 0.45,
                        }}
                      />
                      {/* Gradients */}
                      <defs>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      ...
                      <p className="text-xs text-gray-500 mt-1 font-medium">{data.count > 0 ? `${data.count} files` : 'No files'}</p>
                    </div>
                  </div>
                  {/* Label */}
